...
def _moments(x, axes, shift=None, keepdims=False):
    ''' Wrapper over tensorflow backend call '''
    if K.backend() == 'tensorflow':
        import tensorflow as tf
        return tf.nn.moments(x, axes, shift=shift, keepdims=keepdims)
    elif K.backend() == 'theano':
        import theano.tensor as T

        mean_batch = T.mean(x, axis=axes, keepdims=keepdims)
        var_batch = T.var(x, axis=axes, keepdims=keepdims)
